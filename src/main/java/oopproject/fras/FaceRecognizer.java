...
public class FaceRecognizer {
    ...
    public String recognizeFace(Mat frame) {
        if (!isTrained) {
            return "Unknown";  // Jika belum dilatih, langsung kembalikan "Unknown"
        }

        Mat gray = new Mat();
        Imgproc.cvtColor(frame, gray, Imgproc.COLOR_BGR2GRAY);  // Pastikan frame menjadi grayscale
        MatOfRect faces = new MatOfRect();
        faceDetector.detectMultiScale(gray, faces, 1.1, 3, 0, new Size(30, 30), new Size());

        for (Rect face : faces.toArray()) {
            Mat faceROI = gray.submat(face);  // Ambil area wajah
